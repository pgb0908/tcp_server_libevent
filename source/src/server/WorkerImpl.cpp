...
namespace Server {
    ...
    WorkerPtr ProdWorkerFactory::createWorker(uint32_t index, const std::string& worker_name) {
        ...
        return std::make_unique<WorkerImpl>(std::move(dispatcher), std::move(conn_handler), api_);
    }

    WorkerImpl::WorkerImpl(Event::DispatcherPtr&& dispatcher, Network::ConnectionHandlerPtr handler, Api::Api& api)
            :  dispatcher_(std::move(dispatcher)), handler_(std::move(handler)), api_(api){
...
/*    void WorkerImpl::removeFilterChains(uint64_t listener_tag,
    ...
    void WorkerImpl::start(const Event::PostCb& cb) {
        //ASSERT(!thread_);
        ...
        thread_ = api_.threadFactory().createThread(
                [this, cb]() -> void { threadRoutine(cb); }, options);
    }

/*    void WorkerImpl::initializeStats(Stats::Scope& scope)
...
/*    void WorkerImpl::stopListener(Network::ListenerConfig& listener, std::function<void()> completion) {
    ...
    void WorkerImpl::threadRoutine(const Event::PostCb& cb) {
