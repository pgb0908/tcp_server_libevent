...
/*    Network::SocketSharedPtr ProdListenerComponentFactory::createListenSocket(
        ...
        //ASSERT(socket_type == Network::Socket::Type::Stream ||socket_type == Network::Socket::Type::Datagram);
...
*//*        // First we try to get the socket from our parent if applicable in each case below.
        if (address->type() == Network::Address::Type::Pipe) {
            if (socket_type != Network::Socket::Type::Stream) {
                ...
                //throw EnvoyException(fmt::format("socket type {} not supported for pipes", toString(socket_type)));
                throw std::runtime_error(fmt::format("socket type {} not supported for pipes", toString(socket_type)));
            }
...
/*    DrainingFilterChainsManager::DrainingFilterChainsManager(ListenerImplPtr&& draining_listener,
    ...
    ListenerManagerImpl::ListenerManagerImpl(Server& server,
...
/*    bool ListenerManagerImpl::removeListenerInternal(const std::string& name,
    ...
    void ListenerManagerImpl::startWorkers( std::function<void()> callback) {
...
/*    void ListenerManagerImpl::stopListener(Network::ListenerConfig& listener, std::function<void()> callback) {
