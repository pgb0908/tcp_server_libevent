...
/*    Address::InstanceConstSharedPtr Utility::protobufAddressToAddress(const envoy::config::core::v3::Address& proto_address) {
...
/*    void Utility::addressToProtobufAddress(const Address::Instance& address,
    ...
    Api::IoCallUint64Result Utility::writeToSocket(IoHandle& handle, Buffer::RawSlice* slices,
        ...
        if (send_result.ok()) {
            ...
            std::cout << fmt::format("sendmsg bytes {}", send_result.return_value_) << std::endl;
        } else {
 /*           ENVOY_LOG_MISC(debug, "sendmsg failed with error code {}: {}",
            ...
            std::cout << fmt::format("sendmsg failed with error code {}: {}",  static_cast<int>(send_result.err_->getErrorCode()),
                                     send_result.err_->getErrorDetails()) << std::endl;
        }
        return send_result;
    }
