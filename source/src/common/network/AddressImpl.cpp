...
#include "src/common/common/SafeMemcpy.h"
  ...
  *sin = {AF_INET, sin6->sin6_port, in_v4, {}};
        ...
        StatusOr<Address::InstanceConstSharedPtr> addressFromSockAddr(const sockaddr_storage& ss,
            ...
            switch (ss.ss_family) {
                ...
                default:
                    return absl::InvalidArgumentError("Unexpected sockaddr family");
...
/*            static const bool supported = SocketInterfaceSingleton::get().ipFamilySupported(AF_INET6);
        ...
        PipeInstance::PipeInstance(const std::string& pipe_path, mode_t mode,
            ...
            if (pipe_path.size() >= sizeof(pipe_.address_.sun_path)) {
               // throw EnvoyException(fmt::format("Path \"{}\" exceeds maximum UNIX domain socket path size of {}.", pipe_path,sizeof(pipe_.address_.sun_path)));
            }
            ...
            if (pipe_path[0] == '@') {
             ...
             //   StringUtil::strlcpy(&pipe_.address_.sun_path[0], pipe_path.c_str(),sizeof(pipe_.address_.sun_path));
