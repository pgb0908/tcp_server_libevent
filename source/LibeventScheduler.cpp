...
#include "TimerImpl.h"
#include "SchedulableCallbackImpl.h"
...
SchedulableCallbackPtr LibeventScheduler::createSchedulableCallback(const std::function<void()> &cb) {
...
void LibeventScheduler::registerOnPrepareCallback(LibeventScheduler::OnPrepareCallback &&callback) {
    callback_ = std::move(callback);
    evwatch_prepare_new(libevent_.get(), &onPrepareForCallback, this);
}

void LibeventScheduler::onPrepareForCallback(evwatch *, const evwatch_prepare_cb_info *info, void *arg) {
    // `self` is `this`, passed in from evwatch_prepare_new.
    auto self = static_cast<LibeventScheduler*>(arg);
    self->callback_();
}

void LibeventScheduler::onPrepareForStats(evwatch *, const evwatch_prepare_cb_info *info, void *arg) {
    // `self` is `this`, passed in from evwatch_prepare_new.
    auto self = static_cast<LibeventScheduler*>(arg);
